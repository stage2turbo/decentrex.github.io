...
API.init = function init(callback, allContracts, path, provider, configName, lookbackIn) {
  ...
  this.web3.version.getNetwork((error, version) => {
    ...
    // path
    if (path) {
      this.config.contractKapoEx = path + this.config.contractKapoEx;
      this.config.contractToken = path + this.config.contractToken;
    }

    // contracts
    this.contractKapoEx = undefined;
    this.contractKapoExAddrs = [this.config.contractKapoExAddrs[0].addr];
    if (allContracts) {
      this.contractKapoExAddrs = this.config.contractKapoExAddrs.map(x => x.addr);
    }
    ...
    async.series(
      [
        (callbackSeries) => {
          this.utility.loadContract(
            ...
            this.contractKapoExAddrs[0],
            (err, contract) => {
              this.contractKapoEx = contract;
      ...
      () => {
        callback(null, {
          contractKapoEx: this.contractKapoEx,
...
API.logs = function logs(callback, lookbackIn) {
  this.utility.blockNumber(this.web3, (err, blockNumber) => {
    ...
    async.mapSeries(
      this.contractKapoExAddrs,
      (contractKapoExAddr, callbackMap) => {
        const blocks = Object.values(this.eventsCache)
          .filter(x => x.address === contractKapoExAddr)
          .map(x => x.blockNumber);
...
API.getKapoExBalance = function getKapoExBalance(addr, callback) {
  if (addr.length === 42) {
...
API.getKapoExTokenBalances = function getKapoExTokenBalances(addr, callback) {
  if (addr.length === 42) {
    async.reduce(
      this.config.tokens,
      {},
...
API.getUSDBalance = function getUSDBalance(addr, tokenPrices, callback) {
  async.parallel(
    [
      ...
      (callbackParallel) => {
        API.getKapoExTokenBalances(addr, callbackParallel);
      },
      (callbackParallel) => {
    ...
    (err, results) => {
      const balances = { Wallet: results[0], KapoEx: results[1] };
...
API.addOrderFromEvent = function addOrderFromEvent(event, callback) {
  if (event.event === 'Order' && event.address === this.contractKapoExAddrs[0]) {
...
API.getTrades = function getTrades(callback) {
  ...
  events.forEach((event) => {
    if (event.event === 'Trade' && this.contractKapoExAddrs.indexOf(event.address) >= 0) {
...
API.getDepositsWithdrawals = function getDepositsWithdrawals(callback) {
  ...
  const events = Object.values(this.eventsCache);
  events.forEach((event) => {
    if (event.event === 'Deposit' && this.contractKapoExAddrs.indexOf(event.address >= 0)) {
      ...
      event.event === 'Withdraw' && this.contractKapoExAddrs.indexOf(event.address) >= 0
    ) {
      if (event.args.amount.toNumber() > 0) {
        const token = API.getToken(event.args.token);
        depositsWithdrawals.push({
      ...
      if (balance.lt(new BigNumber(amountGive))) {
        ...
        this.utility.sign(this.web3, addr, hash, pk, (errSign, sig) => {
          if (errSign) {
            ...
            const order = {
              contractAddr: this.contractKapoExAddrs[0],
              tokenGet,
              amountGet,
              tokenGive,
              amountGive,
